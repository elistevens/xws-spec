...
(function() {
  var $exp, _base, _base1, _validateSquadron_nameAndUpgrades, _validateSquadron_pilot, _validateSquadron_upgradeSlot, _validateSquadron_upgrades,
  ...
  $exp.xws.checkVersion = function(other_str) {
    var oMajor, oMinor, oPatch, other_list, vMajor, vMinor, vPatch, _ref;
    other_list = other_str.split('.');
    if (other_list.length !== 3) {
      return false;
    }
    oMajor = other_list[0], oMinor = other_list[1], oPatch = other_list[2];
    _ref = $exp.xws.version.split('.'), vMajor = _ref[0], vMinor = _ref[1], vPatch = _ref[2];
    if (vMajor !== oMajor) {
      return false;
    }
    if (vMinor < oMinor) {
  ...
  _validateSquadron_upgradeSlot = function(slot_key, dirty_obj, prefix, vendor) {
    var clean_list, error, error_list, i, upgrade_key, _i, _len, _ref;
    if (vendor == null) {
      vendor = true;
    }
    try {
  ...
  _validateSquadron_upgrades = function(dirty_obj, prefix, vendor) {
    var clean_list, clean_obj, dirty_key, dirty_value, error, error_list, error_sublist, key2obj_dict, slot_key, _ref, _ref1;
    if (vendor == null) {
      vendor = true;
    }
    try {
      ...
      if (dirty_obj) {
        ...
        for (slot_key in _ref) {
          ...
          clean_list = [];
          if (slot_key in dirty_obj) {
            _ref1 = _validateSquadron_upgradeSlot(slot_key, dirty_obj[slot_key], "" + prefix + "." + slot_key, vendor), clean_list = _ref1[0], error_sublist = _ref1[1];
            if (clean_list) {
  ...
  _validateSquadron_nameAndUpgrades = function(faction_key, ship_key, clean_obj, dirty_obj, prefix, vendor) {
    var dirty_key, dirty_value, error, error_list, error_sublist, _ref;
    if (vendor == null) {
      vendor = true;
    }
    try {
      ...
      if (!dirty_obj.name || !(dirty_obj.name in $exp.xws.pilot_faction2ship2pilot2obj_dict[faction_key].ships[ship_key].pilots)) {
        return [null, ["" + prefix + ".name: " + dirty_obj.name]];
      }
      clean_obj.name = dirty_obj.name;
      delete dirty_obj.name;
      _ref = _validateSquadron_upgrades(dirty_obj.upgrades, prefix, vendor), clean_obj.upgrades = _ref[0], error_sublist = _ref[1];
      error_list = error_list.concat(error_sublist);
  ...
  _validateSquadron_pilot = function(faction_key, dirty_obj, prefix, vendor) {
    var clean_obj, clean_section, dirty_section, error, error_list, error_sublist, i, _i, _len, _ref, _ref1, _ref2;
    if (vendor == null) {
      vendor = true;
    }
    try {
      ...
      if (clean_obj.ship === 'cr90corvette') {
        clean_obj.sections = [];
        _ref = dirty_obj.sections;
        for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
          dirty_section = _ref[i];
          _ref1 = _validateSquadron_nameAndUpgrades(faction_key, clean_obj.ship, {
            upgrades: []
          }, dirty_section, "" + prefix + ".sections[" + i + "]", vendor = true), clean_section = _ref1[0], error_sublist = _ref1[1];
          clean_obj.sections.push(clean_section);
          error_list = error_list.concat(error_sublist);
        }
      } else {
        _ref2 = _validateSquadron_nameAndUpgrades(faction_key, clean_obj.ship, clean_obj, dirty_obj, prefix, vendor = true), clean_obj = _ref2[0], error_sublist = _ref2[1];
        error_list = error_list.concat(error_sublist);
      }
      return [clean_obj, error_list];
    } catch (_error) {
  ...
  $exp.xws.validateSquadron = function(dirty_obj, prefix, vendor) {
    var attr, clean_obj, dirty_key, dirty_value, error, error_list, error_sublist, i, pilot_clean, pilot_dirty, _i, _j, _len, _len1, _ref, _ref1, _ref2;
    if (prefix == null) {
      prefix = 'squadron';
    }
    if (vendor == null) {
    ...
    try {
      ...
      if (!$exp.xws.checkVersion(dirty_obj.version)) {
      ...
      for (i = _j = 0, _len1 = _ref1.length; _j < _len1; i = ++_j) {
        pilot_dirty = _ref1[i];
        _ref2 = _validateSquadron_pilot(clean_obj.faction, pilot_dirty, "" + prefix + ".pilots[" + i + "]", vendor), pilot_clean = _ref2[0], error_sublist = _ref2[1];
        if (pilot_clean) {
