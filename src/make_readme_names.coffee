...
key2listTable = (title, data, cols=['Name', 'Canonical']) ->
    console.log("## #{title}")
    console.log("")
...
key2objnameTable = (title, data, cols) ->
    subdata = {}
    for own key, obj of data
        subdata["#{obj.name} #{key}"] = [obj.name, key]

    key2listTable(title, subdata, cols)

name2keyTable = (title, data, cols) ->
    subdata = {}
    for own key, obj of data
        subdata[key] = [key, obj]
